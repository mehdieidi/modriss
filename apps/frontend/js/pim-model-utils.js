...
export const PIM_NESTED_CONTAINMENTS = Object.freeze({
  ...
  ObjectStore: [{ feature: "notificationRules", types: ["ObjectNotificationRule"] }],
  Topic: [
    {
...
const PIM_REQUIRED_FEATURES = Object.freeze({
  ...
  PIMModel: ["architectureStyle", "services", "deploymentUnits", "environments", "functions"],
  ...
  DataStore: ["storeKind", "consistencyNeed", "ownedDataModels", "accessPatterns"],
...
export function pimRootContainmentForType(type) {
  return (
    PIM_ROOT_CONTAINMENTS.find((entry) =>
...
function inheritedRequiredFeatures(type) {
  ...
  const metadataRequired = [...(definition?.attributes || []), ...(definition?.references || [])]
  ...
  return [
    ...new Set([...supertypeRequired, ...(PIM_REQUIRED_FEATURES[type] || []), ...metadataRequired]),
  ];
}

export function missingRequiredFeatures(element, extraRequired = []) {
  const type = pimTypeOf(element);
  const required = new Set([...inheritedRequiredFeatures(type), ...extraRequired]);
...
function derivedRequiredFeatureIsSatisfied(element, feature) {
  ...
  if (
    (type === "ReadinessFinding" || type === "ReadinessCheck" || type === "ManualDecision") &&
    feature === "assessment"
  ) {
...
export function nestedContainmentsForType(elementType) {
  const entries = [];
  Object.entries(PIM_NESTED_CONTAINMENTS).forEach(([ownerType, containments]) => {
    if (
  ...
  entries.forEach((entry) => {
    ...
    current.types = [...new Set([...current.types, ...(entry.types || [])])].filter(
      (type) => type && !PIM_ABSTRACT_TYPES.includes(type),
    );
    current.relationshipOnly = Boolean(current.relationshipOnly || entry.relationshipOnly);
...
function nestedContainmentCopies(parentId, parentType, graph, feature) {
  ...
  graph.elementsById.forEach((candidate) => {
    ...
    if (
      allowedTypes.size &&
      ![...allowedTypes].some((type) => pimTypeMatches(candidateType, type))
    ) {
      return;
    }
    if (
...
function relationshipBelongsToParentContainment(relationship, parent, parentId, feature, graph) {
  ...
  if (feature === "transitions" && type === "WorkflowTransition") {
    const source = graph.elementsById.get(
      refId(relationship.source) || relationship.sourceElementId,
    );
    const target = graph.elementsById.get(
      refId(relationship.target) || relationship.targetElementId,
    );
...
function attachNestedContainments(copy, elementId, elementType, graph) {
  ...
  nestedContainmentsForType(elementType).forEach((entry) => {
    const value = nestedContainmentCopies(elementId, elementType, graph, entry.feature);
    if (entry.singleton) {
      if (value) {
...
export function populatePimRootContainments(root, graph) {
  ...
  const existingTraceModel =
    root.traceModel && typeof root.traceModel === "object"
...
function normalizeSemanticElement(raw, fallbackType, index, owner = null) {
  ...
  return {
    ...
    id,
    name: String(
      raw.name || raw.label || raw.semanticName || raw.variableName || raw.literal || id,
    ),
    label: String(
      raw.name || raw.label || raw.semanticName || raw.variableName || raw.literal || id,
    ),
  ...
  nestedContainmentsForType(type).forEach((entry) => {
    ...
    const rawValues = entry.singleton
      ? parent?.[entry.feature]
        ? [parent[entry.feature]]
        : []
      : Array.isArray(parent?.[entry.feature])
        ? parent[entry.feature]
        : [];
    rawValues.forEach((raw, index) => {
      const child = normalizeSemanticElement(
        raw,
        raw?.eClass || raw?.type || entry.types[0],
        index,
        { id: parent.id, feature: entry.feature },
      );
      if (!child) {
        return;
      }
  ...
  PIM_ROOT_CONTAINMENTS.forEach((entry) => {
    ...
    const rawValues = entry.singleton
      ...
      : Array.isArray(modelJson[entry.feature])
        ...
        : [];
    rawValues.forEach((raw, index) => {
      const element = normalizeSemanticElement(
        raw,
        raw?.eClass || raw?.type || entry.types[0],
        index,
      );
...
function relationshipFromObject(raw, fallbackType, kind, sourceId, targetId, index, owner = null) {
  if (!sourceId || !targetId) {
    return null;
  }
  const type = raw?.eClass || raw?.type || fallbackType;
  const id = String(
    raw?.id || `rel-pim-${String(kind).toLowerCase()}-${sourceId}-${targetId}-${index + 1}`,
  );
  return {
    ...clone(raw),
    id,
...
function addRootRelationshipObjects(modelJson, result) {
  (Array.isArray(modelJson?.triggers) ? modelJson.triggers : []).forEach((raw, index) => {
    result.push(
  ...
  (Array.isArray(modelJson?.dataAccesses) ? modelJson.dataAccesses : []).forEach((raw, index) => {
    result.push(
      relationshipFromObject(
        raw,
        "DataAccess",
        dataAccessKind(raw.mode),
        refId(raw.function),
        refId(raw.store),
        index,
      ),
  ...
  (Array.isArray(modelJson?.flows) ? modelJson.flows : []).forEach((raw, index) => {
  ...
  traceLinks.forEach((raw, index) => {
    result.push(
      relationshipFromObject(
        ...
        refId(raw.target) || String(raw.targetElementId || ""),
        index,
...
function addNestedRelationshipObjects(parent, result) {
  const type = pimTypeOf(parent);
  if (type === "Topic") {
    (Array.isArray(parent.subscriptions) ? parent.subscriptions : []).forEach((raw, index) => {
      result.push(
        relationshipFromObject(
          raw,
          "Subscription",
  ...
  if (type === "EventBus") {
    (Array.isArray(parent.routingRules) ? parent.routingRules : []).forEach((raw, index) => {
  ...
  if (type === "Workflow") {
    (Array.isArray(parent.transitions) ? parent.transitions : []).forEach((raw, index) => {
      result.push(
        relationshipFromObject(
          raw,
          "WorkflowTransition",
  ...
  if (type === "Principal") {
    (Array.isArray(parent.permissions) ? parent.permissions : []).forEach((raw, index) => {
      result.push(
        relationshipFromObject(
          raw,
          "Permission",
  ...
  nestedContainmentsForType(type)
...
export function pimSemanticEdgeObjectSpec(kind, sourceType, targetType) {
  ...
  if (
    ["READS", "WRITES", "READ_WRITE", "APPEND", "DELETE", "DATA_ACCESS"].includes(normalizedKind) &&
    ...
    return {
      ...
      defaults: {
        mode:
          normalizedKind === "READS"
            ? "READ"
            : normalizedKind === "WRITES"
              ? "WRITE"
              : normalizedKind === "DATA_ACCESS"
                ? "READ_WRITE"
                : normalizedKind,
      },
    };
  ...
  if (
    ["INVOKES", "TRIGGERS"].includes(normalizedKind) &&
    ...
    return {
      ...
      defaults: {
        invocationMode: normalizedKind === "TRIGGERS" ? "ASYNCHRONOUS" : "SYNCHRONOUS",
      },
    };
  }
  if (
  ...
  if (
    flowType &&
    pimTypeMatches(sourceType, "FlowEndpoint") &&
