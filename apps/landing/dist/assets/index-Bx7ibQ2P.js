...
async function b() {
  ...
  try {
    let t = await fetch(_, { signal: e.signal, cache: `no-store` });
    if (!t.ok) return ``;
    if ((t.headers.get(`Content-Type`) || ``).includes(`application/json`)) {
      let e = await t.json();
      return typeof e.ip == `string` ? e.ip : ``;
    }
...
async function x() {
  let e = await b(),
    t = JSON.stringify({
      ...
      attributes: { referrer: document.referrer, publicIp: e },
      occurredAt: new Date().toISOString(),
    }),
    n = `${g}/api/telemetry/frontend`;
  (navigator.sendBeacon && navigator.sendBeacon(n, new Blob([t], { type: `application/json` }))) ||
    fetch(n, {
      method: `POST`,
