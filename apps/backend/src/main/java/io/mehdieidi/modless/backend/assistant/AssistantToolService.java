...
public class AssistantToolService {
    ...
    public AssistantToolService(AssistantCatalogService catalogs,
            AssistantPatchCompiler patchCompiler, ObjectMapper mapper) {
     ...
     */
    @Tool(name = "searchCatalogs", description = "Search compact backend-owned metamodel and EVL catalogs.")
     ...
     */
    @Tool(name = "previewSemanticPatch", description = "Compile and preview a typed semantic patch. Does not commit changes.")
    public PreviewResult previewSemanticPatch(
        ...
        try {
            ...
            SemanticModelPatch semantic = mapper.readValue(
            ...
            return new PreviewResult(compiled.affectedElements(), compiled.patch(),
                    compiled.inversePatch(), preview);
     ...
     */
    @Tool(name = "summarizeValidation", description = "Summarize structural and EVL validation issues without committing changes.")
    public AssistantValidationSummary summarizeValidation(
        ...
        long optional = issues.stream()
                .filter(issue -> "WARNING".equalsIgnoreCase(issue.severity()))
                .count();
        ModelLevel.fromApiName(level);
        return new AssistantValidationSummary(mandatoryPassed, mandatoryPassed, (int) optional,
                issues);
    }
     ...
     */
    @Tool(name = "requestUserChoice", description = "Create a bounded user-choice request for ambiguous modeling intent.")
    public AssistantChoice requestUserChoice(
        ...
        List<AssistantChoice.Option> options = java.util.Arrays.stream(
                        (optionIds == null ? "" : optionIds).split(","))
                .map(String::trim)
        ...
        return new AssistantChoice(id == null || id.isBlank() ? "assistant-choice" : id,
                prompt == null || prompt.isBlank() ? "Choose how to proceed." : prompt, options);
    }
    ...
    private List<AssistantValidationSummary.Issue> parseIssues(String issuesJson) {
        ...
        try {
            return mapper.readValue(issuesJson,
     ...
     */
    public record PreviewResult(List<String> affectedElements,
