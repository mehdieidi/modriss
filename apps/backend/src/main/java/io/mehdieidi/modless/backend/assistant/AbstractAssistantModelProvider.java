...
abstract class AbstractAssistantModelProvider implements AssistantModelProvider {
    ...
    AbstractAssistantModelProvider(String providerKey, AiProperties properties,
    ...
    public AssistantReply complete(AssistantPrompt rawPrompt) {
        ...
        String content = hardening.providerCall(prompt.role(), providerKey, model, () -> {
            var request = chatClient.prompt().options(options(model));
            if (registerTools(prompt.role())) {
                request = request.tools(tools);
            }
            return request.system(SYSTEM_GUARDRAIL + "\n" + prompt.system())
    ...
    public SemanticModelPatch proposePatch(AssistantPrompt rawPrompt) {
        ...
        SemanticModelPatch patch = hardening.providerCall(AssistantModelRole.PLANNER, providerKey,
                model, () -> {
                    var request = chatClient.prompt().options(options(model));
