 ...
 * available. Hash mode keeps tests and constrained local environments deterministic.</p>
 */
@Service
public class LocalAssistantEmbeddingService {
    ...
    public LocalAssistantEmbeddingService() {
        this(new AiProperties.Embeddings(AiProperties.EmbeddingProvider.HASH, null, null, null,
                null, false, -1, true));
    }
    ...
    private Object onnxModel() throws Exception {
        ...
        synchronized (this) {
            ...
            modelClass.getMethod("setDisableCaching", boolean.class)
                    .invoke(model, properties.disableCaching());
            if (properties.gpuDeviceId() >= 0) {
                modelClass.getMethod("setGpuDeviceId", int.class)
                        .invoke(model, properties.gpuDeviceId());
            }
