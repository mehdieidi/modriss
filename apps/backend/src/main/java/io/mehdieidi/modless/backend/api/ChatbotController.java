...
public class ChatbotController {
     ...
     */
    public ChatbotController(AssistantOrchestrator assistant, AssistantRealtimeHub realtime,
            AuthSupport auth, ProjectService projects) {
    ...
    CreateSessionResponse createSession(@RequestHeader("X-Auth-Token") String token,
        ...
        AssistantSessionStore.AssistantSession session = assistant.startSession(user,
                request.projectId(), level, request.modelName());
    ...
    MessageResponse message(@RequestHeader("X-Auth-Token") String token,
            @PathVariable String sessionId,
            @Valid @RequestBody MessageRequest request) {
        UserRecord user = auth.user(token);
        AssistantOrchestrator.AssistantTurnResponse response = assistant.handleMessage(
                user, sessionId,
                new AssistantOrchestrator.AssistantTurnRequest(request.message(),
                        request.modelId(), request.revision(), request.activeView(),
                        request.selectedElementIds(), request.unsavedDraftPatch()));
        return new MessageResponse(response.assistantMessage(), response.modelId(),
                response.revision(), null, response.proposal(), response.choices(),
     ...
     */
    @GetMapping(value = "/api/chatbot/sessions/{sessionId}/events", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    ...
    AssistantProposal proposal(@RequestHeader("X-Auth-Token") String token,
            @PathVariable String sessionId, @PathVariable String proposalId) {
    ...
    MessageResponse approve(@RequestHeader("X-Auth-Token") String token,
            @PathVariable String sessionId, @PathVariable String proposalId) {
        AssistantOrchestrator.AssistantTurnResponse response = assistant.approveProposal(
                auth.user(token), sessionId, proposalId);
        return new MessageResponse(response.assistantMessage(), response.modelId(),
                response.revision(), null, response.proposal(), response.choices(),
                response.workflowState());
    }
    ...
    void reject(@RequestHeader("X-Auth-Token") String token, @PathVariable String sessionId,
    ...
    MessageResponse undo(@RequestHeader("X-Auth-Token") String token,
            @PathVariable String sessionId, @PathVariable String proposalId) {
        AssistantOrchestrator.AssistantTurnResponse response = assistant.undoProposal(
                auth.user(token), sessionId, proposalId);
        return new MessageResponse(response.assistantMessage(), response.modelId(),
                response.revision(), null, response.proposal(), response.choices(),
                response.workflowState());
    ...
    void choose(@RequestHeader("X-Auth-Token") String token, @PathVariable String sessionId,
    ...
    public record CreateSessionRequest(@NotBlank String modelType, String modelName,
                                       String initialDocument, @NotBlank String projectId) {
     ...
     */
    @JsonIgnoreProperties(ignoreUnknown = true)
    public record MessageRequest(@NotBlank String message, String modelId, Long revision,
                                 String activeView, List<String> selectedElementIds,
                                 String unsavedDraftPatch, String attachmentName,
     ...
     */
    public record MessageResponse(String assistantMessage, String modelId, Long revision,
                                  JsonNode model, AssistantProposal proposal,
