...
public class GeminiAssistantModelProvider extends AbstractAssistantModelProvider {
     ...
     */
    public GeminiAssistantModelProvider(AiProperties properties,
            ProxyAvailability proxyAvailability, AssistantPromptGuard promptGuard,
            AssistantToolService tools, AssistantHardeningService hardening) {
        super(AiProperties.Provider.GEMINI.key(), properties, proxyAvailability, promptGuard,
                tools, hardening, ChatClient.create(chatModel(properties)));
    }

    private static GoogleGenAiChatModel chatModel(AiProperties properties) {
        Client client = Client.builder()
                .apiKey(properties.gemini().apiKey().isBlank() ? "not-configured"
        ...
        return GoogleGenAiChatModel.builder()
                ...
                .defaultOptions(GoogleGenAiChatOptions.builder()
                        .model(properties.models().forRole(AiProperties.Provider.GEMINI,
    ...
    private static ClientOptions clientOptions(AiProperties properties) {
        ...
        if (!proxy.enabled() || proxy.type() == AiProperties.ProxyType.DIRECT) {
            ...
            proxyOptions
                    .type(proxy.type() == AiProperties.ProxyType.SOCKS
                            ? ProxyType.Known.SOCKS : ProxyType.Known.HTTP)
                    .host(proxy.host())
                    .port(proxy.port());
        }
