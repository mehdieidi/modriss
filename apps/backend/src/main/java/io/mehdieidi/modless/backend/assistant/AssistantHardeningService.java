...
public class AssistantHardeningService {
     ...
     */
    @Autowired
    public AssistantHardeningService(AiProperties properties,
            @Nullable MeterRegistry meterRegistry) {
        this(properties, meterRegistry, Clock.systemUTC());
    }

    AssistantHardeningService(AiProperties properties, @Nullable MeterRegistry meterRegistry,
            Clock clock) {
    ...
    public void checkRateLimit(String userId) {
        ...
        Window window = windows.compute(userId, (key, current) -> {
            if (current == null || now.isAfter(current.startedAt()
                    .plus(hardening.rateLimitWindow()))) {
     ...
     */
    public <T> T providerCall(AssistantModelRole role, String provider, String model,
            Supplier<T> call) {
        Instant now = clock.instant();
        ...
        try {
            ...
            for (int attempt = 1; attempt <= properties.hardening().providerRetryAttempts();
                    attempt++) {
                try {
                    T result = call.get();
                    circuit = new Circuit(0, Instant.EPOCH);
                    counter("assistant.provider.success", "provider", provider, "role",
            ...
            log.warn("AI provider call failed after retries for provider={} role={} model={}: {}",
                    provider, role, model, last == null ? "unknown" : last.toString());
            throw new PlatformException(502,
                    "AI provider request failed. Check provider base URL, model, API key, and proxy settings.");
        } finally {
            if (sample != null) {
                sample.stop(Timer.builder("assistant.provider.duration")
    ...
    private void recordFailure(String provider) {
        ...
        Instant openUntil = failures >= hardening.circuitFailureThreshold()
