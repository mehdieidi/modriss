...
public final class ModuleDependencyResolver {
  ...
  public List<ModuleDescriptor> sort(Collection<ModuleDescriptor> modules) {
    ...
    ArrayDeque<Path> queue = indegrees.entrySet().stream()
    ...
    if (sorted.size() != modules.size()) {
      throw new IllegalStateException("The Emfatic modules contain a circular dependency through .ecore imports.");
    }
