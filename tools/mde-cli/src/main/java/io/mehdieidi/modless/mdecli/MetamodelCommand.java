...
import io.mehdieidi.modless.mdecli.diagnostics.ConsoleReportWriter;
import io.mehdieidi.modless.mdecli.diagnostics.ConversionReport;
import io.mehdieidi.modless.mdecli.diagnostics.ReportFileWriter;
import io.mehdieidi.modless.mdecli.service.ConversionException;
...
public final class MetamodelCommand implements Callable<Integer> {
    ...
    @Parameters(index = "0", paramLabel = "INPUT", description = "Input .emf/.emfatic file or directory.")
    private Path input;

    @Option(names = {"-o",
            "--output"}, paramLabel = "OUTPUT", description = "Target .ecore file path.")
    private Path output;

    @Option(names = "--root", paramLabel = "ROOT", description = "Root Emfatic file for directory conversion.")
    ...
    public Integer call() {
        ...
        ConversionRequest request = new ConversionRequest(input, output, rootFile, overwrite,
                verbose);
        try {
