...
public final class DiagnosticMapper {
  ...
  public List<DiagnosticEntry> map(Resource resource) {
    ...
    resource.getErrors().forEach(error -> diagnostics.add(fromResourceDiagnostic(error, Severity.ERROR)));
    resource.getWarnings().forEach(warning -> diagnostics.add(fromResourceDiagnostic(warning, Severity.WARNING)));
    return diagnostics;
  }
  ...
  private DiagnosticEntry fromResourceDiagnostic(Resource.Diagnostic diagnostic, Severity severity) {
    return new DiagnosticEntry(
        severity,
