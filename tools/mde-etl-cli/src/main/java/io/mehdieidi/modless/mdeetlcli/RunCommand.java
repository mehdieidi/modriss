...
public final class RunCommand implements Callable<Integer> {
    ...
    @Option(names = {"-m", "--module"}, required = true, description = "ETL module file.")
    private Path module;
    ...
    @Option(names = "--source-metamodel", required = true, split = ",", description = "Source .ecore metamodel file. Repeat or comma-separate.")
    private List<Path> sourceMetamodels;

    @Option(names = "--target-metamodel", required = true, split = ",", description = "Target .ecore metamodel file. Repeat or comma-separate.")
    ...
    @Option(names = "--read-existing-target", description = "Read an existing target model before execution.")
    ...
    public Integer call() throws Exception {
        EtlExecutionRequest request = new EtlExecutionRequest(
                ...
                Arrays.asList(
                        EtlModelConfiguration.source(sourceName, sourceAliases, sourceModel,
                                sourceMetamodels),
                        EtlModelConfiguration.target(targetName, targetAliases, targetModel,
                                targetMetamodels, readExistingTarget)),
                overwrite,
                true);
