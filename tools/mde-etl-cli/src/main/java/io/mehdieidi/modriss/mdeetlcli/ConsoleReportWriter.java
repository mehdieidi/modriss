...
import io.mehdieidi.modriss.mde.etl.EtlDiagnostic;
...
final class ConsoleReportWriter {
  ...
  private void writeTiming(PrintWriter writer, EtlExecutionReport report) {
    ...
    writer.printf(
        "ETL timing: validation=%dms, prepare=%dms, parse=%dms, sourceLoad=%dms, targetLoad=%dms,"
            + " execute=%dms, store=%dms, dispose=%dms, total=%dms%n",
        timing.validationMs(),
        timing.prepareOutputsMs(),
        timing.parseMs(),
        timing.sourceModelLoadMs(),
        timing.targetModelLoadMs(),
        timing.etlExecuteMs(),
        timing.modelStoreMs(),
        timing.disposeMs(),
        timing.totalMs());
