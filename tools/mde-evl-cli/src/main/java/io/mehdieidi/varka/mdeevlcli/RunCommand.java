...
import io.mehdieidi.varka.mde.validation.FileEvlModelConfiguration;
...
public final class RunCommand implements Callable<Integer> {
  ...
  @Option(
      names = "--module",
      split = ",",
      description = "Specific EVL module file(s), relative to --evl-root unless absolute.")
  ...
  @Option(
      ...
      required = true,
      split = ",",
      description = "Ecore metamodel file. Repeat or comma-separate.")
  private List<Path> metamodels;

  @Option(
      names = "--model-name",
      required = true,
      description = "Epsilon model name used by EVL contexts, e.g. PIM.")
        ...
        new EvlValidationRequest(
            ...
            List.of(
                new FileEvlModelConfiguration(
                    ...
                    metamodels.stream().map(path -> path.toAbsolutePath().normalize()).toList(),
