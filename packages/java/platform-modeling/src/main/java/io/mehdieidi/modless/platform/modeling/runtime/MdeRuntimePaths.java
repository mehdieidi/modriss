package io.mehdieidi.modless.platform.modeling.runtime;
...
public final class MdeRuntimePaths {
  ...
  public Path metamodelFile(ModelLevel level) {
    ...
    return root.resolve(level.apiName())
        .resolve(
            switch (level) {
  ...
  private Path resolveConfiguredRoot(Path configured, String defaultRelative) {
    ...
    if (!Files.isDirectory(absolute)) {
      throw new PlatformException(
          500, "Configured MDE asset directory does not exist: " + absolute);
    }
    return absolute;
  }

  /**
   * Resolves the repository root from options or by walking up from the process working directory.
   *
  ...
  private Path resolveRepositoryRoot() {
    ...
    throw new PlatformException(
        500, "MDE assets were not found from the backend working directory.");
  }
  ...
  private void requireRepositoryRoot(Path root) {
    if (!isRepositoryRoot(root)) {
      throw new PlatformException(
          500,
          "Configured MDE repository root is missing required "
              + "validation, transformation, generation, or metamodel assets: "
              + root);
    }
  }
  ...
  private boolean isRepositoryRoot(Path root) {
    return Files.isRegularFile(root.resolve("mde/validation/cim/cim-semantic-validation.evl"))
        && Files.isRegularFile(root.resolve("mde/validation/pim/pim-semantic-validation.evl"))
        && Files.isRegularFile(root.resolve("mde/validation/psm/psm-semantic-validation.evl"))
        && Files.isRegularFile(root.resolve("mde/transformations/cim-to-pim/cim-to-pim.etl"))
        && Files.isRegularFile(root.resolve("mde/transformations/pim-to-awspsm/pim-to-awspsm.etl"))
        && Files.isRegularFile(
            root.resolve("mde/generation/awspsm-to-artifacts/awspsm2artifacts.egx"))
