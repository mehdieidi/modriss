package io.mehdieidi.modriss.platform.modeling.xmi;
...
public final class XmiModelImportService {
   ...
   *
   * <p>Several MODRISS metamodels define {@code id} as their Ecore ID attribute, while generated
   * XMI commonly omits {@code xmi:id}. EMF's default fallback resolves those references by walking
   * the complete resource and calling {@code EcoreUtil.getID} for every lookup. Large generated
   * models therefore become quadratic during load. The deferred resolver runs after the document
   * has been materialized, so the complete intrinsic-ID index can be built once and reused.
   */
  ...
  private final class SerializationContext {
    ...
    private ObjectNode shallowGraphElement(ObjectNode semanticNode) {
      ...
      copyLayoutAnnotation(semanticNode, element, "modriss.layout.x", "x");
      copyLayoutAnnotation(semanticNode, element, "modriss.layout.y", "y");
      return element;
    }
