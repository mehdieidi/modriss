...
import io.mehdieidi.modriss.platform.assistant.agent.AgentActionSchema;
...
public class OpenAiCompatibleAssistantModelProvider extends AbstractAssistantModelProvider {
  ...
  protected OpenAiChatOptions.Builder options(
      ...
      io.mehdieidi.modriss.platform.assistant.provider.AssistantModelProvider.AssistantPrompt
          prompt) {
                  ...
                  name ->
                      (org.springframework.ai.tool.ToolCallback)
                          FunctionToolCallback.<String, String>builder(name, arguments -> arguments)
                              .description(
                                  "MODRISS assistant tool; backend validates and executes it.")
        ...
        throw new PlatformException(
            ...
            response.statusCode() >= 400 && response.statusCode() < 500
                ? "AI provider rejected the configured tool protocol. Check "
                    + "MODRISS_AI_OPENAI_PROTOCOL, the selected model, and endpoint capabilities."
                    + errorBody
  ...
  private static String toolArgumentsSchema(
      String name,
      java.util.List<
              io.mehdieidi.modriss.platform.assistant.metamodel.MetamodelKnowledgeService
                  .TypeContract>
          patchContracts) {
    try {
