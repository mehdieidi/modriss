...
- Express every requested relationship using its exact writable non-containment reference. In the conceptual blueprint, record the LLM-chosen feature and target as `plannedReferences`; an unlabelled target ID or shared containment is not relationship evidence. When the DSML represents a relationship as an EClass, create it under its valid owner and connect every required endpoint. Apply this uniformly to CIM and PIM relationship kinds, not only workflow transitions.
