...
Canonical, machine-readable **iterative-incremental** modeling process definitions for CIM, PIM, AWS PSM, and the end-to-end pipeline. Each process follows **SPEM 2.0**: sequential phases → stages → (optional sub-stages) → atomic tasks, plus roles, artifact kinds, guidelines, and a **process engine** (the agile kernel that delivers increments).
