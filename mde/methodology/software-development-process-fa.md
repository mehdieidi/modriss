# فرایند توسعهٔ نرم‌افزار مدل‌محور سرورلس MODRISS

> **وضعیت سند:** تعریف مبنای فرایند (Process Baseline) برای متدولوژی MODRISS
...
> **زبان و ابزار مدل‌محور:** Ecore/EMF، EVL، ETL، EGX/EGL و پلتفرم کم‌کد MODRISS

## ۱. هدف و جایگاه این سند

متدولوژی MODRISS از دو جزء مکمل تشکیل می‌شود:
...
مقاله فعالیت‌های چتری را در سراسر چرخهٔ عمر نشان می‌دهد، اما خود تصریح می‌کند که الگوهای ریزدانهٔ آن‌ها را به‌طور کامل استخراج نکرده است. در این سند، این شکاف با فعالیت‌های لازم برای یک پروژهٔ واقعی و با توجه به شواهد فنی MODRISS تکمیل شده است؛ بنابراین نباید جزئیات فعالیت‌های چتری این سند را عیناً به MDASP نسبت داد.

### ۲.۲. نقش نمونهٔ فارسی

فصل فرایند پیشنهادی Deljouyi نمونه‌ای از انطباق یک چارچوب مدل‌محور با فرایند کامل توسعه است. از آن، شیوهٔ ارائهٔ نقش‌ها، محصولات هر فاز، ساخت تکراری–افزایشی، آزمون کوچک/بزرگ، CI/CD، مسیرهای بازگشت و جداسازی انتقال از نگهداری اقتباس شده است. اجزای خاص دامنهٔ آن مانند هستان‌شناسی، YAML و REST به MODRISS منتقل نشده‌اند و با CIM/PIM/AWS PSM و زنجیرهٔ واقعی ابزارهای پروژه جایگزین شده‌اند.
...
### ۲.۴. نگاشت MDASP به MODRISS

| الگوی MDASP                   | تحقق یا انطباق در MODRISS                                                      |
| ----------------------------- | ------------------------------------------------------------------------------ |
...
| Define Infrastructure         | آماده‌سازی MODRISS، مخزن، محیط‌ها، AWS/LocalStack، CI/CD و روش کار             |
...
| T01   | پلتفرم MODRISS                  | ابزار اجرای مدل‌سازی، ذخیره، اعتبارسنجی، تبدیل، تولید و کاوش مصنوعات      |
...
- ایجاد پروژه، عضویت‌ها و سطح دسترسی در MODRISS؛
...
موتور ماشین‌خوان `modriss.end-to-end.engine` ترتیب عمودی سطوح را محقق می‌کند. زیرفرایند
`modriss.artifact.deployment-readiness` نیز از بازرسی خط‌مبنای تولیدشده در این فاز آغاز می‌شود و
...
زیرفرایند رسمی `modriss.cim.modeling` اجرا می‌شود:
...
زیرفرایند `modriss.pim.modeling` اجرا می‌شود:
...
زیرفرایند `modriss.psm.modeling` اجرا می‌شود:
