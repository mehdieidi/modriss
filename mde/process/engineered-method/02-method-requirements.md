...
| MR-LC-08 | Separate planned delivery from unpredictable operational demand while keeping both connected to one product lifecycle and evidence system.                                                          | MUST  | Release/increment planning and an interrupt-driven service flow are distinct SPEM activities with explicit hand-off rules.           |
| MR-LC-09 | Manage operational work as a pull system with a defined workflow, WIP controls, explicit policies, service-level expectations, and flow measures.                                                   | MUST  | The method profile and operations board expose states, WIP, policies, SLEs, item age, cycle time, throughput, and blockers.          |
...
| MR-MG-08 | Govern capacity and preemption across planned delivery and operational demand without pretending that future incidents are schedulable.                | MUST   | The method profile defines operational capacity policy, replenishment, WIP, expedite rules, and how selected work enters a planned release. |
