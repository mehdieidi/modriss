...
### Executable role bindings

The integrated process compiles each task's accountable primary performer and
its required supporting performers into distinct SPEM ProcessPerformer uses.
Supporting means that the role contributes evidence, review, or a decision; it
does not require every named role to attend every work session. The generated
role ledger verifies that all 17 stable technical RoleDefinitions have at
least one executable binding, including Sponsor, FinOps/Cost Analyst, and
Records/Data Steward.
...
The executable process may use a cohesive run record while the conceptual
library distinguishes independently owned products. Such a compiled record
must retain explicit `methodWorkProductIds`; aggregation never removes the
canonical owner, acceptance criteria, provenance, or retention obligation.
...
| WP-30 | Service Work Item                            | Service Owner         | source, affected service, maintenance purpose where applicable, emergency-temporary status, class of service, severity, owner, SLE, state, age, evidence, authoritative re-entry point, disposition      |
| WP-31 | Kanban Service-Delivery Policy and Board     | Delivery Lead         | Requested/Ready/In Progress/Verify/Done states, start/finish points, WIP controls, pull/replenishment rules, classes of service, SLEs, capacity and expedite policies, cadences, board and flow measures |
