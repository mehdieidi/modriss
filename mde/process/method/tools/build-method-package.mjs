...
const packageRoot = path.join(root, 'mde', 'process', 'method');
const sourceRoot = path.join(root, 'mde', 'process', 'definitions');
...
for (const sourceName of sourceNames) {
  ...
  for (const task of tasks) {
    ...
    const primaryRole = task.primaryPerformerRoleRef ?? task.performerRoleRefs?.[0];
    const supportingRoles = (task.performerRoleRefs ?? []).filter(roleRef => roleRef !== primaryRole);
    const roleSentence = supportingRoles.length
      ? `Its primary performer is ${mdRefs([primaryRole])}; supporting performers are ${mdRefs(supportingRoles)}.`
      : `Its primary performer is ${mdRefs([primaryRole])}.`;
    catalog.push(`${purpose.endsWith('.') ? purpose : `${purpose}.`} ${roleSentence} It consumes ${mdRefs(task.inputWorkProductRefs)} and produces or updates ${mdRefs(task.outputWorkProductRefs)}.`, '');
...
const contentElementXml = (element, type, depth) => {
  ...
  if (type === 'WorkProductDefinition') {
    ...
    for (const methodWorkProductId of element.methodWorkProductIds ?? (element.methodWorkProductId ? [element.methodWorkProductId] : [])) {
      lines.push(line(depth + 1, `<modriss:methodWorkProductId>${escapeXml(methodWorkProductId)}</modriss:methodWorkProductId>`));
    }
  ...
  if (type === 'TaskDefinition') {
    ...
    for (const roleRef of element.performerRoleRefs ?? []) {
      const performerKind = roleRef === (element.primaryPerformerRoleRef ?? element.performerRoleRefs?.[0]) ? 'primary' : 'supporting';
      lines.push(line(depth + 1, `<modriss:defaultPerformer roleDefinitionRef="${xmiId(roleRef)}" modriss:kind="${performerKind}"/>`));
    }
...
const processXml = (source, depth) => {
  ...
  for (const milestone of model.milestones ?? []) {
    lines.push(line(depth + 1, `<nestedBreakdownElement xmi:type="spem:Milestone" xmi:id="${xmiId(milestone.id)}" name="${escapeXml(milestone.name ?? milestone.id)}" modriss:sourceId="${escapeXml(milestone.id)}"${attr('modriss:gateId', milestone.gateId)}${attr('modriss:phaseRef', milestone.phaseId ? xmiId(milestone.phaseId) : null)}${attr('modriss:stageRef', milestone.stageId ? xmiId(milestone.stageId) : null)}${attr('modriss:acceptanceCondition', milestone.acceptanceCondition)}>`));
    for (const roleRef of milestone.authorityRoleRefs ?? []) lines.push(line(depth + 2, `<modriss:authorityRoleRef>${escapeXml(roleRef)}</modriss:authorityRoleRef>`));
    for (const evidenceRef of milestone.requiredEvidenceRefs ?? []) lines.push(line(depth + 2, `<modriss:requiredEvidenceRef>${escapeXml(evidenceRef)}</modriss:requiredEvidenceRef>`));
    lines.push(line(depth + 1, `</nestedBreakdownElement>`));
  }
