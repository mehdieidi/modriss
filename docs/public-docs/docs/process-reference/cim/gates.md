# CIM modeling: gates and milestones

This page documents the **SPEM Milestone with MODRISS evidence and authority extensions** elements used by the CIM modeling process. It is generated from the canonical executable definition, so the names and identifiers match the API and process interface. The explanation adds practical teaching around the normative data.
...
| --------------- | ----------------------- | ----------: | ----------------: |
| cim.m1.evl-gate | CIM EVL validation gate |           0 |                 0 |

## Detailed gates and milestones

## CIM EVL validation gate

<small>Milestone: `cim.m1.evl-gate` · phase/activity `cim.ph5`</small>

The responsible authorities accept the required evidence.
