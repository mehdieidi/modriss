...
| `VARKA_AI_REQUEST_TIMEOUT`                | Positive duration; sample `240s`                             | Maximum time for one provider request. Keep it below the applicable turn timeout. The Gemma source workflow can require more than 180 seconds for a large blueprint response.                                |
...
| `VARKA_AI_PREFER_LLM_SOURCE_EXTRACTION`   | `true`, `false`; deployed `true`                             | Uses the LLM for source-evidence extraction. Keep `true` for the required fully LLM-based assistant; `false` is a compatibility/testing mode, not the validated production profile.                          |
| `VARKA_AI_LLM_REVIEW_ENABLED`             | `true`, `false`; deployed `true`                             | Adds independent LLM blueprint-completeness and generated-obligation reviews. These are semantic review passes, while the publication gate remains structural Ecore/EMF conformance only.                    |
...
| `VARKA_AI_MAX_PATCH_CREATES`              | Positive integer; sample `64`                                | Maximum creates accepted in one inspect/contract agent patch. The conceptual blueprint is separately capped at 96 objects.                                                                                   |
...
| `VARKA_AI_MAX_PROVIDER_CALLS_PER_TURN`    | Positive integer; Gemma profile `64`                         | Hard turn-wide provider-call ceiling. The staged conceptual path uses the extra headroom for Gemma slice correction, durable per-item recovery, and independent LLM review while every call remains audited. |
| `VARKA_AI_MAX_PROVIDER_CALLS_SOURCE_TURN` | Positive integer; full-source profile `96`                   | Provider-call ceiling for source-backed turns. This accommodates a large Gemma blueprint, bounded slice repairs, and both review passes; increase only with matching timeout/cost monitoring.                |
| `VARKA_AI_SOURCE_TURN_TIMEOUT`            | Positive duration; sample `25m`                              | Overall timeout for source-backed turns. It should be at least as large as the normal turn timeout.                                                                                                          |
...
| `VARKA_AI_MODEL`             | `Gemma-4-31B-IT` for production, or another configured model name          | Single model used by strategy, conceptual, and agent calls. The deployed Arvan configuration requires `Gemma-4-31B-IT`.                                |
