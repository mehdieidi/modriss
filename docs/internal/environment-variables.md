...
The conceptual blueprint schema maximum (18 objects/types), obligation-ledger maximum (12), four
...
| `VARKA_AI_LLM_REVIEW_ENABLED`             | `true`, `false`; deployed `false`                            | Adds an independent LLM obligation judge. The validated Gemma profile disables it to preserve call/latency budget; structural Ecore checks and obligation allocation still apply.   |
...
| `VARKA_AI_MAX_PATCH_CREATES`              | Positive integer; sample `48`                                | Maximum creates accepted in one inspect/contract agent patch. It does not change the separate conceptual 18-object schema cap or its lower effective capacity.                      |
...
| `VARKA_AI_MAX_PROVIDER_CALLS_PER_TURN`    | Positive integer; tested `24`                                | Hard turn-wide provider-call ceiling. The staged conceptual path uses the extra headroom for Gemma slice correction while every call remains audited.                               |
