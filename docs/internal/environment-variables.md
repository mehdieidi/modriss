...
| `MODLESS_AI_MODE`                        | `AUTONOMOUS`, `GUARDED_APPLY`                                | Assistant rollout mode. `AUTONOMOUS` is the current behavior. `GUARDED_APPLY` is accepted as a legacy value and currently behaves like autonomous mode. |
...
| `MODLESS_AI_MAX_TOOL_CALLS`              | `0` or positive integer                                      | Legacy total tool/operation limit. `0` or negative means unbounded in the backend code.                                                                 |
| `MODLESS_AI_VALIDATION_REPAIR_ATTEMPTS`  | Positive integer                                             | How many times the assistant may try to repair a model change after validation fails.                                                                   |
| `MODLESS_AI_MAX_AGENT_STEPS`             | Positive integer                                             | Maximum planner loop iterations for one assistant mutation turn.                                                                                        |
| `MODLESS_AI_MAX_TOOL_CALLS_PER_STEP`     | Positive integer                                             | Maximum tool calls allowed during one planner step.                                                                                                     |
| `MODLESS_AI_MAX_AUTO_APPLY_OPERATIONS`   | `0` or positive integer                                      | Legacy auto-apply operation limit. `0` or negative means unbounded in the backend code.                                                                 |
| `MODLESS_AI_SEMANTIC_VALIDATION_ENABLED` | `true`, `false`                                              | Whether assistant turns must enforce semantic validation. Leave off unless you are testing stricter behavior.                                           |
...
| `MODLESS_AI_CONTEXT_WINDOW_TOKENS`       | `0` or positive integer                                      | Provider/model context window when aliases cannot self-report. `0` means "let the app/provider defaults decide."                                        |
...
| `MODLESS_AI_FALLBACK_PROVIDER`           | Empty, `openai`, `gemini`                                    | Optional backup provider used after rate-limit responses from the main provider. Empty means no fallback.                                               |
