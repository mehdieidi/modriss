...
| `MODLESS_AI_MODELING_STRATEGY`           | `model-subset`, `semantic-patch`                             | How assistant model changes are drafted. `model-subset` compiles JSON Schema guided partial subsets into validated semantic patches.                  |
| `MODLESS_AI_REQUEST_TIMEOUT`             | Duration like `5m`, `10m`                                    | Maximum time to wait for one provider request before giving up.                                                                                       |
| `MODLESS_AI_MAX_TOOL_CALLS`              | `0` or positive integer                                      | Legacy total tool/operation limit. Bound in config but **not enforced** by the orchestrator (`0` normalizes to unbounded).                            |
...
| `MODLESS_AI_MAX_AGENT_STEPS`             | Positive integer                                             | Bound in config but **not enforced** by the orchestrator today.                                                                                       |
| `MODLESS_AI_MAX_TOOL_CALLS_PER_STEP`     | Positive integer                                             | Bound in config but **not enforced** by the orchestrator today.                                                                                       |
| `MODLESS_AI_SEMANTIC_VALIDATION_ENABLED` | Documented in `.env.example` only                            | **Not implemented** — assistant apply gate uses Ecore structural validation, not EVL                                                                  |
| `MODLESS_AI_MAX_AGENT_STEPS`             | Bound in `application.yml`                                   | **Not enforced** in the current orchestrator                                                                                                          |
| `MODLESS_AI_MAX_TOOL_CALLS`              | Bound in `application.yml`                                   | **Not enforced** (`0` means unbounded in config normalization)                                                                                        |
| `MODLESS_AI_MAX_TOOL_CALLS_PER_STEP`     | Bound in `application.yml`                                   | **Not enforced**                                                                                                                                      |
