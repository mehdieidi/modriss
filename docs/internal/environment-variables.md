...
| `VARKA_AI_MAX_PROVIDER_CALLS_PER_TURN`    | Positive integer; full-source profile `40`                   | Hard turn-wide provider-call ceiling. The staged conceptual path uses the extra headroom for Gemma slice correction and independent LLM review while every call remains audited.            |
| `VARKA_AI_MAX_PROVIDER_CALLS_SOURCE_TURN` | Positive integer; full-source profile `64`                   | Provider-call ceiling for source-backed turns. This accommodates a 64-object Gemma blueprint, bounded repairs, and both review passes; increase only with matching timeout/cost monitoring. |
