...
The full MDE lifecycle is `modless.end-to-end.modeling` (`mde/methodology/process-definitions/end-to-end.json`). One SPEM phase (`e2e.ph1`) contains **eight stages** that orchestrate child process engines, ETL transforms, EVL gates, M2T, and cross-level rework loops.
...
flowchart LR
    ...
    p1["e2e.p1.cim-modeling<br/>modless.cim.modeling · 5 phases"]
    evl1["cim-semantic-validation<br/>e2e.m1.cim-ready"]
    p2["e2e.p2.cim-to-pim"]
    p3["e2e.p3.pim-refinement<br/>modless.pim.modeling · 6 phases"]
    evl2["pim-semantic-validation<br/>e2e.m2.pim-ready"]
    p4["e2e.p4.pim-to-psm"]
    p5["e2e.p5.psm-refinement<br/>modless.psm.modeling · 6 phases"]
...
| ---------------------------- | ---------------------- | ----------------------- |
| `e2e.p1.cim-modeling`        | `modless.cim.modeling` | 5 (`cim.ph1`–`cim.ph5`) |
| `e2e.p2.cim-to-pim`          | `cim-to-pim` ETL       | —                       |
| `e2e.p3.pim-refinement`      | `modless.pim.modeling` | 6 (`pim.ph1`–`pim.ph6`) |
| `e2e.p4.pim-to-psm`          | `pim-to-awspsm` ETL    | —                       |
| `e2e.p5.psm-refinement`      | `modless.psm.modeling` | 6 (`psm.ph1`–`psm.ph6`) |
